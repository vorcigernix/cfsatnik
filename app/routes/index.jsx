/* eslint-disable no-undef */
import { json, useLoaderData, redirect } from "remix";

export const loader = async ({ params }) => {
  const entries = await SATNIK.list({ prefix: "new:F:", limit: 10 });
  const data = await Promise.all(entries.keys.map(key => SATNIK.get(key.name, {type: "json"})));
  return data;
};

export default function Index() {
  const hadry = useLoaderData();
  console.log(hadry);
  return (
    <section className="text-gray-600 body-font overflow-hidden">
      <div className="container px-5 py-24 mx-auto">
        <div className="-my-8 divide-y-2 divide-gray-100">
          {/* looop */}
          {hadry.map((item, index) => {
            return (
              <div className="py-8 flex flex-wrap md:flex-nowrap" key={index}>
                <div className="md:w-64 md:mb-0 mb-6 flex-shrink-0 flex flex-col">
                  <span className="font-semibold title-font text-gray-700">
                    {item.sex}
                  </span>
                  <span className="mt-1 text-gray-500 text-sm">
                    {item.status}
                  </span>
                </div>
                <div className="md:flex-grow">
                  <h2 className="text-2xl font-medium text-gray-900 title-font mb-2">
                    {item.name}
                  </h2>
                  <p className="leading-relaxed">{item.description}</p>
                  <a
                    className="text-red-500 inline-flex items-center mt-4"
                    href="/"
                  >
                    Fetch
                    <svg
                      className="w-4 h-4 ml-2"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      strokeWidth="2"
                      fill="none"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                    >
                      <path d="M5 12h14"></path>
                      <path d="M12 5l7 7-7 7"></path>
                    </svg>
                  </a>
                </div>
              </div>
            );
          })}
          {/* looop */}
        </div>
      </div>
    </section>
  );
}
